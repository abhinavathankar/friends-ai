...
# --- Instagram Logic ---
# --- Instagram Logic (Real AI Image Generation) ---
# Note: Using Google's Imagen via Gemini API
...
def generate_imagen_image(topic, chat_log, api_key):
    ...
    # Note: As of late 2024/2025, model name might be 'imagen-3.0-generate-001' or similar
    # We will try the standard call structure
    
    # Construct a prompt for the visual
    dialogue_text = "\n".join([f"{entry['agent']}: {entry['message']}" for entry in chat_log if entry['agent'] not in ['Manager', 'Critic']])
    
    prompt = f"""
    Create a high-quality, realistic iPhone screenshot of an iMessage group chat named 'Friends Syndicate ☕️'.
    Dark mode interface.
    
    The conversation is about '{topic}'.
    
    The messages visible are:
    {dialogue_text}
    
    Participants: Joey (Gray bubble), Chandler (Blue bubble), Ross (Gray bubble).
    Ensure the text is legible and looks like a real specialized iOS screenshot.
    """
    
    try:
        # Implementation based on standard Library integration
        # Note: 'imagen-3.0-generate-001' is the target, but wrapped in genai
        # Currently, the python lib uses:
...
if st.button("📸 Create Instagram Post (Imagen AI)", type="secondary", use_container_width=True):
    if st.session_state.chat_log:
        if not api_key:
            st.error("You need a Gemini API Key to Generate AI Images!")
        else:
            with st.spinner("Dreaming up the screenshot with Imagen 3..."):
                # Generate Image
                img = generate_imagen_image(st.session_state.topic, st.session_state.chat_log, api_key)
                
                if img:
                    # Show in UI
                    st.image(img, caption="Generated by Google Imagen", use_container_width=True)
                    ...
                    st.download_button(
                        ...
                        data=byte_im,
                        file_name="friends_imagen.png",
                        mime="image/png"
                    )
                else:
                    st.error("Image generation failed or model not available for this key scope.")
    else:
